...
export async function GenerateDepth(workspace: string) {
    return new Promise((resolve, reject) => {
        fs.writeFile(path.join(workspace, 'base', 'depth.ts'), DepthFile, (err) => {
            if (err) {
                reject(err);
            } else {
                resolve();
...
export async function GenerateHL7Obj(workspace: string) {
    return new Promise((resolve, reject) => {
        fs.writeFile(path.join(workspace, 'base', 'HL7Obj.ts'), DepthFile, (err) => {
            if (err) {
                reject(err);
            } else {
                resolve();
